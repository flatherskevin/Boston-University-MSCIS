...
class BaseballStatsDAO(AbstractDAO):
    ...
    def insert_records(self, records):
        """
        Inserts records into the database determined by instance variable db_name

        :param records: list of BaseballStatRecord()
        :return: None
        """
        # Connect to database
        ...
        # Insert each record into its own row in the database
    ...
    def select_all(self):
        """
        Selects all the data in the database and creates BaseballStatRecord() objects for
        every row selected.

        :return: deque of all BaseballStatRecord() objects
        """
        # Connect to database
        current_db = self.connect()
        ...
        # For every row selected, create a BaseballStatRecord()
        for record in cursor:
            records.append(BaseballStatRecord(record[0], record[3], record[1], record[2]))

        # Close the connection and return the deque
        current_db.close()
        return records
...
class StocksDAO(AbstractDAO):
    ...
    def insert_records(self, records):
        ...
        :param records: list of StockStatRecord()
        :return: None
        """
        # Connect to database
        current_db = self.connect()
    ...
    def select_all(self):
        """
        Selects all the data in the database and creates StockStatRecord() objects for
        every row selected.

        :return: deque of all StockStatRecord() objects
        """
        # Connect to database
        current_db = self.connect()
...
# Add some styling to make it easier to read information in the console
print('\n<> <> <> <> <> <> <> <> <> <>')
print('\nRESULTS:  STOCKS\n')
print('<> <> <> <> <> <> <> <> <> <>\n')

# Create a reading object and a DAO object for stocks
stocks_records = StockCSVReader('./StockValuations.csv')
stocks_DAO = StocksDAO()

# Handle any errors when inserting data
# This allows the program to continue if the primary key has already been used
try:
    stocks_DAO.insert_records(stocks_records.load())
except Exception as err:
    print(err)

# Implement DAO.select_all() method
all_stocks = stocks_DAO.select_all()

# Initialize a list to hold unique values of exchange country
unique_exchange_country_list = []

# Initialize a dictionary to hold number of tickers per exchange country
exchange_country_tickers_dict = {}

# Loop through all the stocks and append unique exchange countries to the list
...
# Count the numbers of tickers per exchange country and add to the dictionary
...
# Go ahead and print all dictionary items to the console
...
# Create a reading object and a DAO object for baseball
...
# Initialize a list to hold unique values of batting average
unique_avg_list = []

# Initialize a dictionary to hold average salary per batting average
avg_salary_dict = {}

# Loop through all the stocks and append unique batting averages to the list
for item in all_baseball:
    if round(item.avg, 3) not in unique_avg_list:
        unique_avg_list.append(round(item.avg, 3))

# Calculate the average salary per battering average and add to the dictionary
